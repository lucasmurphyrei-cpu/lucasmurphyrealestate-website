...
const NeighborhoodQuizSection = ({
  ...
  if (isDark) {
    return (
      <section id="quiz" className="my-0 rounded-2xl border border-white/10 bg-white/[0.04] p-6 md:p-10">
        <div className="flex items-center gap-3 mb-3">
          <div className="rounded-lg p-2" style={{ backgroundColor: "hsl(44,100%,53%,0.15)" }}>
            <Compass className="h-6 w-6" style={{ color: GOLD }} />
          </div>
          <h2 className="font-display text-2xl font-bold text-white">{heading}</h2>
        </div>
        <p className="text-sm text-white/60 mb-6 max-w-2xl">{subtitle}</p>
          ...
          {!isOpen ? (
            ...
            >
              <button
                ...
                className="inline-flex items-center gap-2 rounded-sm px-7 py-3 text-sm font-semibold text-[#0a1424] transition-transform duration-300 hover:-translate-y-0.5"
